 ...
 * Handles Arweave blockchain connection with Warp Contracts
 */
...
/**
 * Global Arweave configuration cache
 * Prevents multiple initialization calls
 */
let cachedArweaveConfig: ArweaveConfig | null = null;

/**
 * Gets the cached Arweave configuration, initializing it if necessary
 *
 * @returns Promise<ArweaveConfig> - The Arweave configuration
 */
export async function getArweaveConfig(): Promise<ArweaveConfig> {
	if (cachedArweaveConfig) {
		return cachedArweaveConfig;
	}

	cachedArweaveConfig = await initializeArweave();
	return cachedArweaveConfig;
}
